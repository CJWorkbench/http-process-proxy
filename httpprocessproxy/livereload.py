...
LIVERELOAD_BYTES = importlib.resources.read_binary(__package__,
                                                   'livereload.js')
PORT = 35729
...
async def _notify(self):
    tasks = {websocket.send(json.dumps({'command': 'reload', 'path': '/'}))
...
async def serve(bind_host: str):
    ...
    """
    server = websockets.serve(_handle_livereload, bind_host, PORT,
